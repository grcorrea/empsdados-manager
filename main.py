...
class AWSApp:
    ...
    def create_sidebar(self):
        ...
        sections = [
            ...
            {"name": "Simular Custo", "icon": ft.Icons.CALCULATE, "id": "CostSimulator", "type": "page"},
            {
                "name": "Report",
                ...
                "subitems": [
                    ...
                    {"name": "Glue", "icon": ft.Icons.DASHBOARD, "id": "Report Glue"}
                ]
            },
            {
                "name": "Monitoring",
    ...
    def on_sidebar_click(self, section_id):
        ...
        if self.current_section != section_id:
            ...
            elif section_id in ["Report Athena", "Report Glue"]:
            ...
            elif section_id == "CostSimulator":
                def auto_update():
                    time.sleep(0.2)  # Pequena pausa para garantir que a UI foi carregada
                    self.auto_update_simulator_data()
    ...
    def update_content_area(self):
        ...
        elif self.current_section == "CostSimulator":
            content = self.cost_simulator_tab
        elif self.current_section == "Report Athena":
            content = self.report_athena_tab
        elif self.current_section == "Report Glue":
            content = self.report_glue_tab
        else:
            content = self.login_tab
    ...
    def setup_layout_with_sidebar(self):
        ...
        self.report_glue_tab = self.create_report_glue_tab()
        self.cost_simulator_tab = self.create_cost_simulator_tab()
...
"""
    ...
    def create_report_glue_tab(self):
        """Cria a aba de relatórios de custos do Glue"""

        # Filtro de período
        self.glue_period_dropdown = ft.Dropdown(
            label="Período",
            width=150,
            options=[
                ft.dropdown.Option("daily", "Diário"),
            ...
            value="monthly",
            on_change=self.on_glue_period_change
        )

        # Filtro de tipo de job
        self.glue_job_type_dropdown = ft.Dropdown(
            label="Tipo de Job",
            width=200,
            options=[
                ft.dropdown.Option("all", "Todos os Jobs"),
                ft.dropdown.Option("ETL", "ETL Jobs"),
                ft.dropdown.Option("STREAMING", "Streaming Jobs"),
                ft.dropdown.Option("PYTHON_SHELL", "Python Shell")
            ],
            value="all",
            on_change=self.on_glue_job_type_change
        )

        # Seletor de data de início
        self.glue_start_date = ft.TextField(
            label="Data Início (YYYY-MM-DD)",
            width=180,
            value="2024-01-01"
        )
        ...
        # Seletor de data de fim
        self.glue_end_date = ft.TextField(
            label="Data Fim (YYYY-MM-DD)",
            width=180,
            value="2024-12-31"
        )
        ...
        # Progress e status
        self.glue_progress = ft.ProgressRing(visible=False)
        self.glue_status = ft.Text(
            "Faça login primeiro para visualizar relatórios de custos",
            size=14,
            color=ft.Colors.GREY_400
        )
        ...
        # Container para gráficos
        self.glue_charts_container = ft.Container(
            content=ft.Text(
                "Selecione o período e clique em 'Gerar Relatório' para visualizar os custos",
                size=16,
                text_align=ft.TextAlign.CENTER,
        ...
        # Tabela de custos detalhados
        self.glue_costs_table = ft.DataTable(
            columns=[
                ft.DataColumn(ft.Text("Job Name", weight=ft.FontWeight.BOLD)),
                ft.DataColumn(ft.Text("Tipo", weight=ft.FontWeight.BOLD)),
                ft.DataColumn(ft.Text("Custo (USD)", weight=ft.FontWeight.BOLD)),
                ft.DataColumn(ft.Text("Execuções", weight=ft.FontWeight.BOLD)),
                ft.DataColumn(ft.Text("DPU Hours", weight=ft.FontWeight.BOLD)),
            ],
            rows=[],
            expand=True
        )

        self.glue_table_container = ft.Container(
            content=self.glue_costs_table,
            expand=True,
            bgcolor=ft.Colors.GREY_800,
            border=ft.border.all(1, ft.Colors.GREY_700),
            border_radius=12,
        ...
        return ft.Container(
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Row([
                            self.glue_period_dropdown,
                            ft.Container(width=15),
                            self.glue_job_type_dropdown,
                            ft.Container(width=15),
                            self.glue_start_date,
                            ft.Container(width=15),
                            self.glue_end_date,
                        ], alignment=ft.MainAxisAlignment.START),
                        ...
                        ft.Row([
                            self.load_glue_jobs_button,
                            ft.Container(width=15),
                            self.refresh_button_glue,
                            self.glue_progress,
                        ], alignment=ft.MainAxisAlignment.START),
                    ]),
                    padding=ft.padding.all(20),
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Text("Relatório de Custos Glue:", size=14, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                        ft.Container(height=10),
                        self.glue_charts_container,
                    ]),
                    expand=True
                ),
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Text("Detalhamento por Job:", size=14, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                        ft.Container(height=10),
                        self.glue_table_container,
                    ]),
                    height=300
                ),
            ],
    ...
    # Report Glue Functions
    def on_glue_period_change(self, e):
        """Callback para mudança de período"""
        print(f"Período selecionado: {self.glue_period_dropdown.value}")

    def on_glue_job_type_change(self, e):
        """Callback para mudança de tipo de job"""
        print(f"Tipo de job selecionado: {self.glue_job_type_dropdown.value}")
        ...
        try:
            ...
            def load_in_background():
                try:
                    ...
                    # Atualizar dropdown com jobs encontrados
                    job_options = [ft.dropdown.Option("all", "Todos os Jobs")]
                    for job in jobs:
                        job_name = job.get('Name', 'Unknown')
                        job_options.append(ft.dropdown.Option(job_name, job_name))
        ...
        try:
            ...
            def generate_in_background():
                try:
                    ...
                    self.glue_status.value = "🚧 Funcionalidade em desenvolvimento"
                    self.glue_status.color = ft.Colors.ORANGE
    ...
    def create_cost_simulator_tab(self):
        """Cria a aba de simulação de custos AWS para sa-east-1"""

        # ============ ATHENA CALCULATOR ============

        # Input de dados escaneados (TB)
        self.athena_data_scanned = ft.TextField(
            label="Dados Escaneados (TB)",
            width=200,
            value="1.0",
            suffix_text="TB",
        ...
        # Botão calcular Athena
        self.calc_athena_button = ft.ElevatedButton(
            "💰 Calcular Athena",
            on_click=self.calculate_athena_cost,
            width=150,
            height=40,
            style=ft.ButtonStyle(
                shape=ft.RoundedRectangleBorder(radius=8),
        ...
        # Resultado Athena
        self.athena_cost_result = ft.Text(
        ...
        # Tipo de máquina Glue
        self.glue_machine_type = ft.Dropdown(
            label="Tipo de Máquina",
            width=200,
            options=[
                ft.dropdown.Option("G.025X", "G.025X (0.25 DPU)"),
                ft.dropdown.Option("G.1X", "G.1X (1 DPU)"),
                ft.dropdown.Option("G.2X", "G.2X (2 DPU)"),
                ft.dropdown.Option("Standard", "Standard (1 DPU)")
            ],
            value="G.1X"
        )

        # Quantidade de DPUs
        self.glue_dpu_count = ft.TextField(
            label="Quantidade DPUs",
            width=150,
            value="2",
            suffix_text="DPU",
            keyboard_type=ft.KeyboardType.NUMBER
        ...
        self.glue_execution_time = ft.TextField(
        ...
        self.calc_glue_button = ft.ElevatedButton(
            "💰 Calcular Glue",
            on_click=self.calculate_glue_cost,
            width=150,
            height=40,
            style=ft.ButtonStyle(
                shape=ft.RoundedRectangleBorder(radius=8),
        ...
        # Status da cotação (atualização automática)
        self.currency_status = ft.Text(
            "Carregando cotação USD/BRL...",
            size=13,
            color=ft.Colors.ORANGE,
            weight=ft.FontWeight.W_500
        )

        # Status de preços AWS (atualização automática)
        self.aws_pricing_status = ft.Text(
            "Carregando preços AWS...",
            size=13,
            color=ft.Colors.ORANGE,
            weight=ft.FontWeight.W_500
        )
        ...
        self.simulator_progress = ft.ProgressRing(visible=False, width=30, height=30)

        # Variáveis internas para cálculos
        self.usd_to_brl_rate = 5.20  # Taxa padrão
        self.athena_last_cost_usd = 0
        self.glue_last_cost_usd = 0

        # Preços AWS sa-east-1 (podem ser atualizados via AI)
        self.aws_pricing = {
            "athena_per_tb": 5.00,  # USD por TB escaneado
            "glue_pricing": {
                "G.025X": 0.11,  # USD por DPU-hora
                "G.1X": 0.44,    # USD por DPU-hora
                "G.2X": 0.88,    # USD por DPU-hora
                "Standard": 0.44  # USD por DPU-hora
            }
        }
            ...
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text(
                            "Região SA-East-1 (São Paulo) • Cotação e Preços Atualizados Automaticamente",
                            size=14,
                ...
                ft.Container(
                    content=ft.Row([
                        ft.Icon(ft.Icons.CURRENCY_EXCHANGE, color=ft.Colors.GREEN, size=20),
                        self.currency_status,
                        ft.Container(expand=True),
                        ft.Icon(ft.Icons.CLOUD, color=ft.Colors.PURPLE, size=20),
                        self.aws_pricing_status,
                        ft.Container(width=10),
                        self.simulator_progress,
                    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                    padding=ft.padding.all(15),
                    bgcolor=ft.Colors.GREY_800,
                    border_radius=10,
                ...
                # Calculadoras lado a lado com design melhorado
                ft.Row([
                    # ============ ATHENA CALCULATOR ============
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Container(
                                content=ft.Row([
                                    ft.Icon(ft.Icons.SEARCH, color=ft.Colors.BLUE, size=24),
                                    ft.Text(
                                        "Amazon Athena",
                                        size=18,
                                        weight=ft.FontWeight.BOLD,
                            ...
                            # Inputs Athena
                            ft.Column([
                                ft.Text("Volume de Dados Escaneados:",
                                        size=13,
                                        color=ft.Colors.BLUE_200,
                                        weight=ft.FontWeight.W_500),
                                ft.Container(height=8),
                                self.athena_data_scanned,
                                ft.Container(height=20),
                                self.calc_athena_button,
                            ...
                            ft.Container(
                                content=ft.Column([
                                    ft.Text("💳 Resultado do Cálculo:",
                                            size=14,
                                            weight=ft.FontWeight.BOLD,
                                            color=ft.Colors.WHITE),
                                    ft.Container(height=10),
                                    self.athena_cost_result,
                                ]),
                                padding=ft.padding.all(15),
                    ...
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Container(
                                content=ft.Row([
                                    ft.Icon(ft.Icons.SETTINGS, color=ft.Colors.GREEN, size=24),
                                    ft.Text(
                                        "AWS Glue",
                                        size=18,
                                        weight=ft.FontWeight.BOLD,
                            ...
                            # Inputs Glue organizados
                            ft.Column([
                                ft.Row([
                                    ft.Column([
                                        ft.Text("Tipo de Máquina:",
                                                size=13,
                                                color=ft.Colors.GREEN_200,
                                                weight=ft.FontWeight.W_500),
                                        ft.Container(height=8),
                                        self.glue_machine_type,
                                    ], expand=1),
                                    ft.Container(width=15),
                                    ft.Column([
                                        ft.Text("Quantidade DPUs:",
                                                size=13,
                                                color=ft.Colors.GREEN_200,
                                                weight=ft.FontWeight.W_500),
                                        ft.Container(height=8),
                ...
                # Seção de comparação melhorada
                ft.Container(
                    content=ft.Column([
                        ft.Row([
                            ft.Icon(ft.Icons.ANALYTICS, color=ft.Colors.ORANGE, size=28),
                            ft.Text(
        ...
        try:
            ...
            def update_all_data():
                try:
                    ...
                    try:
                        import requests
                        response = requests.get("https://api.awesomeapi.com.br/json/last/USD-BRL", timeout=8)
                        data = response.json()
                    ...
                    if self.athena_last_cost_usd > 0 or self.glue_last_cost_usd > 0:
            ...
            # Executar atualizações em background
    ...
    def calculate_athena_cost(self, e):
        ...
        try:
            data_tb = float(self.athena_data_scanned.value or 0)

            # Custo em USD
            cost_usd = data_tb * self.aws_pricing["athena_per_tb"]
            cost_brl = cost_usd * self.usd_to_brl_rate

            self.athena_last_cost_usd = cost_usd
    ...
    def calculate_glue_cost(self, e):
        """Calcula o custo do Glue baseado no tipo de máquina, DPUs e tempo"""
        try:
            machine_type = self.glue_machine_type.value
            dpu_count = int(self.glue_dpu_count.value or 0)
            execution_time_minutes = float(self.glue_execution_time.value or 0)
    ...
    def update_comparison(self):
        """Atualiza a seção de comparação entre Athena e Glue"""
        if self.athena_last_cost_usd > 0 or self.glue_last_cost_usd > 0:
            ...
            if self.glue_last_cost_usd > 0:
                comparison_text += f"⚙️ AWS Glue:\n"
                comparison_text += f"   💵 ${self.glue_last_cost_usd:.2f} USD\n"
                comparison_text += f"   💰 R$ {glue_brl:.2f} BRL\n\n"

            if self.athena_last_cost_usd > 0 and self.glue_last_cost_usd > 0:
                comparison_text += "📈 Análise de Economia:\n"

                if self.athena_last_cost_usd < self.glue_last_cost_usd:
            ...
            comparison_text += f"\n\n💱 Taxa de Conversão: 1 USD = R$ {self.usd_to_brl_rate:.3f}"

            self.comparison_result.content = ft.Text(
                comparison_text,
                size=13,
                text_align=ft.TextAlign.LEFT,
                color=ft.Colors.WHITE,
